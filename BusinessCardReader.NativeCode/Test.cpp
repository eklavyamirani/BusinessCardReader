...
IplImage * textDetection(IplImage * input, bool dark_on_light)
{
	...
	cvSaveImage("gaussian.png", gaussianImage);
	for (int i = 0; i < gaussianImage->height; i++)
	{
		for (int j = 0; j < gaussianImage->width; j++)
		{
			float val = CV_IMAGE_ELEM(gaussianImage, float, i, j);
			int k = 0;
		}
	}
...
int main()
{
	IplImage* inputImage = cvLoadImage(
		"C:\\Users\\Eklavya\\documents\\visual studio 2013\\Projects\\TextDetectcpp\\Debug\\BC5.jpg");
	if (!inputImage)
	{
